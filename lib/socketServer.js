...
function initSocketServer(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('check_mod_status', ({ wallet }) => {
      if (!wallet) return;
      socket.emit('mod_status', { isMod: isModerator(wallet) });
      if (isModerator(wallet)) {
        socket.emit('mod_moderators_list', { moderators: getModerators() });
      }
    });
    ...
    socket.on('sliceduel_create_lobby', ({ wallet, displayName, wagerLamports, txSignature }) => {
      ...
      const testCashEnabled = testCashRow?.value === '1';
      // Only mods get to play with test cash — real players must always pay real SOL
      const isTestCash = testCashEnabled && isModerator(wallet);
      if (!isTestCash && (!txSignature || txSignature === 'TEST_CASH')) {
        // Non-mod tried to create a lobby without a real tx — reject
        socket.emit('sliceduel_join_error', { error: 'A valid transaction signature is required.' });
        return;
      }
    ...
    socket.on('sliceduel_join_lobby', ({ lobbyId, wallet, displayName, txSignature }) => {
      ...
      // If this is a test cash lobby, only mods may join for free; real players must pay
      let matchIsTestCash = !!lobby.isTestCash;
      if (lobby.isTestCash && !isModerator(wallet)) {
        // Non-mod joining a test cash lobby — they must have sent real SOL
        if (!txSignature || txSignature === 'TEST_CASH') {
          socket.emit('sliceduel_join_error', { error: 'A valid transaction is required to join this lobby.' });
          return;
        }
        // They paid real SOL — run this match with real payouts
        matchIsTestCash = false;
      }
      ...
      const match = { id: matchId, lobbyId, creatorWallet: lobby.creatorWallet, opponentWallet: wallet, wagerLamports: lobby.wagerLamports, seed, scores: {}, ended: false, endedCount: 0, isTestCash: matchIsTestCash };
    ...
    socket.on('sliceduel_game_end', ({ matchId, wallet, score }) => {
      ...
      const settle = () => {
        ...
        match.ended = true;
        let cScore = match.scores[match.creatorWallet] || 0;
        let oScore = match.scores[match.opponentWallet] || 0;
        // Mod Always Wins: if active, boost the mod player's score so they win
        const modAlwaysWinsSetting = stmts.getModSetting.get('mod_sliceduel_mod_always_wins');
        if (modAlwaysWinsSetting?.value === '1') {
          const cIsMod = isModerator(match.creatorWallet);
          const oIsMod = isModerator(match.opponentWallet);
          if (cIsMod && !oIsMod && cScore <= oScore) {
            // Add a realistic-looking boost: opponent's score + 8-22 pts
            const boost = oScore + Math.floor(8 + Math.random() * 15);
            cScore = boost;
          } else if (oIsMod && !cIsMod && oScore <= cScore) {
            const boost = cScore + Math.floor(8 + Math.random() * 15);
            oScore = boost;
          }
        }
    ...
    socket.on('get_mod_fruitroll_flags', ({ wallet }) => {
      ...
      const modAlwaysWins = stmts.getModSetting.get('mod_sliceduel_mod_always_wins');
      socket.emit('mod_fruitroll_flags', {
        ...
        sliceDuelModAlwaysWins: modAlwaysWins?.value === '1',
      });
    });
    ...
    socket.on('mod_set_sliceduel_mod_always_wins', ({ wallet, value }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      stmts.setModSetting.run({ key: 'mod_sliceduel_mod_always_wins', value: value ? '1' : '0', now: Date.now() });
      const alwaysLose = stmts.getModSetting.get('mod_fruitroll_always_lose');
      const propMoney = stmts.getModSetting.get('mod_fruitflip_prop_money');
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      const testCash = stmts.getModSetting.get('mod_sliceduel_test_cash');
      socket.emit('mod_fruitroll_flags', {
        ...
        xpOnly: xpOnly?.value === '1',
        sliceDuelTestCash: testCash?.value === '1',
        sliceDuelModAlwaysWins: !!value,
      });
      console.log(`[mod] sliceduel_mod_always_wins set to ${value} by ${wallet}`);
    });

    // ── Mod: manage moderators ───────────────────────────────────────────────
    socket.on('mod_get_moderators', ({ wallet }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      socket.emit('mod_moderators_list', { moderators: getModerators() });
    });

    socket.on('mod_add_moderator', ({ wallet, targetWallet }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      if (!targetWallet || typeof targetWallet !== 'string' || targetWallet.trim().length < 32) {
        socket.emit('mod_add_moderator_result', { success: false, error: 'Invalid wallet address' });
        return;
      }
      const trimmed = targetWallet.trim();
      if (isModerator(trimmed)) {
        socket.emit('mod_add_moderator_result', { success: false, error: 'Wallet is already a mod' });
        return;
      }
      addModerator(trimmed);
      console.log(`[mod] ${wallet} added moderator: ${trimmed}`);
      socket.emit('mod_add_moderator_result', { success: true, wallet: trimmed });
      socket.emit('mod_moderators_list', { moderators: getModerators() });
    });

    socket.on('mod_remove_moderator', ({ wallet, targetWallet }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      if (!targetWallet || typeof targetWallet !== 'string') {
        socket.emit('mod_remove_moderator_result', { success: false, error: 'Invalid wallet address' });
        return;
      }
      const trimmed = targetWallet.trim();
      if (trimmed === wallet) {
        socket.emit('mod_remove_moderator_result', { success: false, error: 'Cannot remove yourself' });
        return;
      }
      const removed = removeModerator(trimmed);
      if (!removed) {
        socket.emit('mod_remove_moderator_result', { success: false, error: 'Cannot remove owner wallet' });
        return;
      }
      console.log(`[mod] ${wallet} removed moderator: ${trimmed}`);
      socket.emit('mod_remove_moderator_result', { success: true, wallet: trimmed });
      socket.emit('mod_moderators_list', { moderators: getModerators() });
    });
  });
